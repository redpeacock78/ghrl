...
app.get("/:author/:repo", async (req): Promise<any> => {
  ...
  if (!tag) {
    ...
    return await tagging({ author: author, repo: repo, source: source, tag: tag })
      .then((i: string) => {
        return {
          statusCode: 302,
          redirect: i,
...
app.get("/:author/:repo/:assets_name", async (req): Promise<any> => {
  ...
  if (!tag) {
    ...
    return await latest({ author: author, repo: repo, assets_name: assets_name })
      .then((i: string) => {
        return {
          statusCode: 302,
          redirect: i,
    ...
    return await tagging({ author: author, repo: repo, tag: tag, assets_name: assets_name })
      .then((i: string) => {
        return {
          statusCode: 302,
          redirect: i,

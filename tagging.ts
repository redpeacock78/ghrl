...
export async function tagging(
  ...
  const tagging_api: string =
    `https://api.github.com/repos/${author}/${repo_name}/releases`;
  return await ky
    .get(tagging_api)
    .then(async (i: Response): Promise<string> => {
      const resp = (await i.json()) as unknown as [
        {
          tag_name: string;
          assets: unknown[];
          tarball_url: string;
      ...
      const tag_arr: string[] = resp.map((
        i: { tag_name: string; assets: unknown[] },
      ): string => i.tag_name);
      if (tag_arr.indexOf(tag) !== -1) {
        const releases_urls: string[] = (resp[tag_arr.indexOf(tag)].assets as [
          { browser_download_url: string },
        ]).map((i): string => {
          return i.browser_download_url;
        });
        if (assets_name) {
          const assets_arr: string[] = releases_urls.map(
            (i: string): string => {
              return i.split("/")[8];
            },
          );
